...
new_dataset = dataset_anotator.annotate(csv_path="dataset-test/reviews.csv")
dataset_anotator.write_dataframe_csv(new_dataset,"dataset-test/reviews_anotated_new.csv")


filter_dataset = Filter_dataset(csv_path="dataset-test/reviews_anotated_new.csv")

# if a parmeter is setted to True, then that don t filter this parameter
sentiments = ['positive']
emotions= ['joy'] # if empty list 
sources = ['google map']

dataset_filtered = filter_dataset.filter(emotions, sentiments, sources , date = None )

dataset_anotator.write_dataframe_csv(dataset_filtered,"dataset-test/dataset_filtered.csv")
